...
def registrar_usuario(email_user, pass_user, proveedor_solicitud=False, proveedor_fecha_solicitud=None, id_rol=1):
    try:
        with db.obtener_conexion() as conexion:
            with conexion.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO usuario (email, pass, id_rol, proveedor_solicitud, proveedor_aprobado, proveedor_fecha_solicitud) VALUES (%s, %s, %s, %s, %s, %s)",
                    (email_user, pass_user, id_rol, int(proveedor_solicitud), 0, proveedor_fecha_solicitud))
