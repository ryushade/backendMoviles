...
import services.manga_service as manga_service
from models.Manga import Libro

def libro_completo():
    try:
        libros = manga_service.obtener_libro_completo()
        if libros:
...
def libro_por_isbn(isbn_lib):
    try:
        libro = manga_service.obtener_libro_por_isbn(isbn_lib)
        if libro:
...
def libros_comprados_por_usuario(email_user):
    try:
        libros = manga_service.obtener_libros_comprados_por_usuario(email_user)
        if libros:
            libros_json = [Libro(*libro).json() for libro in libros]
...
def libros_mas_vendidos():
    try:
        libros = manga_service.obtener_libros_mas_vendidos()
        if libros:
            libros_json = [Libro(*libro).json() for libro in libros]
            return jsonify({"code": 0, "msg": "Libros obtenidos correctamente", "data": libros_json}), 200
        else:
...
def libros_antiguos():
    try:
        libros = manga_service.obtener_libros_antiguos()
        if libros:
            libros_json = [Libro(*libro).json() for libro in libros]
...
def libros_actuales():
    try:
        libros = manga_service.obtener_libros_actuales()
        if libros:
            libros_json = [Libro(*libro).json() for libro in libros]
...
def obtener_preview_libro_por_id(libro_id):
    try:
        libro = manga_service.obtener_detalle_libro_por_isbn(libro_id)
        if libro:
            return jsonify({
                "code": 0,
...
def mejores_autores_libros():
    try:
        autores, libros = manga_service.obtener_libros_mejores_autores()
